...
class TokenStorage:
    ...
    def store_tokens(self, server_url: str, tokens: dict) -> None:
        """Store tokens (legacy method - now just stores the access_token as API key)."""
        if 'access_token' in tokens:
            self.store_api_key(tokens['access_token'])
    
    def get_tokens(self, server_url: str) -> Optional[dict]:
