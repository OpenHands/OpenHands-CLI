...
try:
    ...
    from openhands.storage.data_models.conversation_metadata import (  # type: ignore[import-not-found]
...
class ConversationManager:
            ...
            if os.path.isdir(conversation_path):
                try:
                    ...
                    created_at = datetime.fromtimestamp(
                        os.path.getctime(conversation_path)
                    )
                            ...
                            with open(metadata_file) as f:
                                ...
                                if "last_updated_at" in metadata:
                                    last_updated_at = datetime.fromisoformat(
                                        metadata["last_updated_at"].replace(
                                            "Z", "+00:00"
                                        )
                                    )
    ...
    def _extract_title_from_conversation(self, conversation_file: str) -> str | None:
    ...
    def list_conversations(self, limit: int = 10) -> None:
        ...
        if not conversations:
            ...
            print_formatted_text(
                HTML("<grey>Start a new conversation by typing a message!</grey>")
            )
            return
        ...
        for i, conv in enumerate(conversations[:limit]):
            ...
            print_formatted_text(
                HTML(f"  <white>{conv.short_id}</white> - {title_display}")
            )
            date_info = (
                f"    <grey>Created: {created_display}, "
                f"Updated: {updated_display}</grey>"
            )
            print_formatted_text(HTML(date_info))
            print_formatted_text("")

        if len(conversations) > limit:
            remaining = len(conversations) - limit
            print_formatted_text(
                HTML(f"<grey>... and {remaining} more conversations</grey>")
            )
            print_formatted_text("")

        print_formatted_text(
            HTML("<grey>Use '/load <id>' to resume a conversation</grey>")
        )
        print_formatted_text("")

    def load_conversation(self, conversation_id: str) -> object | None:
        ...
        for conv in conversations:
            if (
                ...
                or conv.conversation_id.startswith(conversation_id)
            ):
                matching_conv = conv
                break
        ...
        if not matching_conv:
            print_formatted_text(
                HTML(f"<red>Conversation '{conversation_id}' not found.</red>")
            )
            print_formatted_text(
                HTML("<grey>Use '/list' to see available conversations.</grey>")
            )
            return None

        try:
            # We need to reconstruct the conversation with the agent
            # For now, we'll return None and let the caller handle creating
            # a new conversation. This is a limitation that would need agent
            # configuration to fully implement
            print_formatted_text(
                HTML(f"<yellow>Found conversation {matching_conv.short_id}</yellow>")
            )
            print_formatted_text(
                HTML(f"<grey>Title: {matching_conv.title or 'Untitled'}</grey>")
            )
            created_date = matching_conv.format_date(matching_conv.created_at)
            print_formatted_text(HTML(f"<grey>Created: {created_date}</grey>"))
            print_formatted_text(
                HTML(
                    "<red>Note: Full conversation loading requires "
                    "agent reconfiguration.</red>"
                )
            )
            print_formatted_text(
                HTML("<grey>This feature will be enhanced in a future update.</grey>")
            )
    ...
    def get_conversation_suggestions(self, partial_id: str) -> list[str]:
        ...
        for conv in conversations:
            ...
            if (
                conv.short_id.startswith(partial_id)
                and conv.short_id not in suggestions
            ):
    ...
    def view_conversation(
        ...
        for conv in conversations:
            if (
                conv.conversation_id == conversation_id
                or conv.short_id == conversation_id
                or conv.conversation_id.startswith(conversation_id)
            ):
                target_conv = conv
                break
        ...
        if not events:
            msg = (
                f"<yellow>No events found in conversation "
                f"{target_conv.short_id}</yellow>"
            )
        ...
        self._display_conversation_header(
            target_conv, total_events, len(paginated_events), offset, event_filter
        )
        ...
        if total_events > offset + limit:
            ...
            msg = (
                f"<grey>... and {remaining} more events. "
                f"Use '/view {conversation_id} --offset {next_offset}' "
                f"to see more</grey>"
            )
            print_formatted_text(HTML(msg))
    ...
    def _filter_events(self, events: list[dict], event_filter: str) -> list[dict]:
        ...
        for event in events:
            ...
            if (
                filter_lower in event_type
                or filter_lower in source
                or self._matches_event_category(event, filter_lower)
            ):
    ...
    def _matches_event_category(self, event: dict, filter_term: str) -> bool:
        ...
        if filter_term == "action" and any(
            action in event_type for action in action_types
        ):
            return True
        elif filter_term == "observation" and any(
            obs in event_type for obs in observation_types
        ):
            return True
        elif filter_term == "command" and "cmdrun" in event_type:
            return True
        elif filter_term == "file" and any(
            file_op in event_type for file_op in ["fileedit", "fileread", "filewrite"]
        ):
            return True
    ...
    def _display_conversation_header(
        self,
        ...
        print_formatted_text(
            HTML(f"<grey>ID: {conv.short_id} ({conv.conversation_id})</grey>")
        )
        print_formatted_text(
            HTML(f"<grey>Modified: {conv.format_date(conv.last_updated_at)}</grey>")
        )

        if filter_type:
            msg = (
                f"<grey>Filter: {filter_type} | Showing {shown} of {total} "
                f"events (offset: {offset})</grey>"
            )
            print_formatted_text(HTML(msg))
        else:
            print_formatted_text(
                HTML(
                    f"<grey>Showing {shown} of {total} events (offset: {offset})</grey>"
                )
            )
    ...
    def _display_event(self, event: dict, index: int):
        ...
        # Source color
        source_color = (
            "cyan" if source == "user" else "yellow" if source == "agent" else "white"
        )

        # Display event header
        header = (
            f"<grey>[{index:3d}]</grey> <{type_color}>{icon} {event_type}"
            f"</{type_color}> <{source_color}>({source})</{source_color}> "
            f"<grey>{time_str}</grey>"
        )
