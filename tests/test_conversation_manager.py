...
class TestConversationInfo:
    ...
    def test_format_date_hours(self):
        ...
        now = datetime.now(UTC)
        created_at = datetime(now.year, now.month, now.day, now.hour - 2, tzinfo=UTC)
        info = ConversationInfo("test-id", created_at)
        formatted = info.format_date(created_at)
...
class TestConversationManager:
    ...
    @patch("openhands_cli.conversation_manager.PERSISTENCE_DIR")
    def test_extract_title_from_conversation_no_user_messages(
        self, mock_persistence_dir
    ):
    ...
    def test_filter_events_by_category(self):
        ...
        assert (
            len(action_events) == 4
        )  # MessageAction, CmdRunAction, FileEditAction, AgentThinkAction
    ...
    def test_extract_event_content(self):
        ...
        event3 = {
            ...
            "args": {"path": "/test/file.py", "content": "print('hello')"},
        }
        content = manager._extract_event_content(event3)
        assert content in [
            "/test/file.py",
            "print('hello')",
        ]  # Either could be returned first
