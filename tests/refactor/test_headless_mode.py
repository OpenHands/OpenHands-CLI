...
class TestHeadlessArgumentParsing:
    ...
    def test_headless_requires_task_or_file(self):
        ...
        # Parser should accept --headless without task/file (validation is in simple_main)
...
class TestHeadlessAppBehavior:
    ...
    async def test_headless_mode_flag_set(self, monkeypatch: pytest.MonkeyPatch):
        ...
        with patch("openhands_cli.refactor.textual_app.OpenHandsApp.run") as mock_run:
    ...
    async def test_conversation_state_change_no_exit_in_non_headless(
        ...
        """Test that conversation completion doesn't trigger exit in non-headless mode."""
        monkeypatch.setattr(
            SettingsScreen,
            "is_initial_setup_required",
            lambda: False,
    ...
    async def test_headless_mode_runs_with_headless_parameter(
        ...
        with patch("openhands_cli.refactor.textual_app.OpenHandsApp") as mock_app_class:
            ...
            # Should set headless_mode flag
            assert mock_app.headless_mode is True
...
class TestHeadlessIntegration:
    ...
    @patch("openhands_cli.agent_chat.run_cli_entry")
    def test_simple_main_validation_rejects_headless_without_task_or_file(self, mock_run_cli_entry):
        """Test that simple_main validation rejects --headless without --task or --file."""
        test_args = ["openhands", "--headless"]
...
class TestConversationSummary:
    ...
    def test_conversation_summary_parsing(self):
        ...
        type(agent_event).visualize = PropertyMock(return_value=Mock(__str__=Mock(return_value="This is a test agent response message.")))
        
        mock_conversation.state.events = [user_event, agent_event, user_event, agent_event]
