...
class TestCLIArgsConfirmation:
    ...
    def test_default_confirmation_policy(self):
        ...
        app = OpenHandsApp()
        assert app.always_approve is False
        assert app.llm_approve is False

    def test_always_approve_argument(self):
        """Test that --always-approve sets the correct flags."""
        app = OpenHandsApp(always_approve=True)
        assert app.always_approve is True
        assert app.llm_approve is False

    def test_llm_approve_argument(self):
        """Test that --llm-approve sets the correct flags."""
        app = OpenHandsApp(llm_approve=True)
        assert app.always_approve is False
        assert app.llm_approve is True

    def test_mutually_exclusive_arguments(self):
        """Test that both arguments can be set (though CLI parser prevents this)."""
        # Note: The CLI parser makes these mutually exclusive, but the app can handle both
        app = OpenHandsApp(always_approve=True, llm_approve=True)
        assert app.always_approve is True
        assert app.llm_approve is True
    ...
    def test_conversation_runner_gets_never_confirm_policy(
        ...
        """Test that --always-approve results in NeverConfirm policy."""
        app = OpenHandsApp(always_approve=True)
    ...
    def test_conversation_runner_gets_confirm_risky_policy(
        ...
        """Test that --llm-approve results in ConfirmRisky policy."""
        app = OpenHandsApp(llm_approve=True)
...
class TestMainFunctionArgs:
    ...
    def test_main_function_passes_always_approve(self, mock_app_class):
        """Test that main function passes always_approve argument to OpenHandsApp."""
        mock_app = MagicMock()
        ...
        # Verify OpenHandsApp was called with always_approve=True
        mock_app_class.assert_called_once()
        call_kwargs = mock_app_class.call_args[1]
        assert call_kwargs["always_approve"] is True
        assert call_kwargs["llm_approve"] is False

    @patch("openhands_cli.refactor.textual_app.OpenHandsApp")
    def test_main_function_passes_llm_approve(self, mock_app_class):
        """Test that main function passes llm_approve argument to OpenHandsApp."""
        mock_app = MagicMock()
        mock_app_class.return_value = mock_app
        mock_app.conversation_id = uuid.uuid4()
        ...
        # Verify OpenHandsApp was called with llm_approve=True
        mock_app_class.assert_called_once()
        call_kwargs = mock_app_class.call_args[1]
        assert call_kwargs["always_approve"] is False
        assert call_kwargs["llm_approve"] is True

    @patch("openhands_cli.refactor.textual_app.OpenHandsApp")
    def test_main_function_default_args(self, mock_app_class):
        """Test that main function uses default values for confirmation args."""
        mock_app = MagicMock()
        mock_app_class.return_value = mock_app
        mock_app.conversation_id = uuid.uuid4()
        ...
        # Verify OpenHandsApp was called with default values
        mock_app_class.assert_called_once()
        call_kwargs = mock_app_class.call_args[1]
        assert call_kwargs["always_approve"] is False
        assert call_kwargs["llm_approve"] is False