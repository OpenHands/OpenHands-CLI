...
@pytest.mark.asyncio
async def test_streaming_regular_content(acp_agent, mock_connection, mock_streaming_chunk):
...
@pytest.mark.asyncio
async def test_streaming_reasoning_content(acp_agent, mock_connection, mock_streaming_chunk):
...
async def test_conversation_setup_enables_streaming(acp_agent):
    ...
    with patch("openhands_cli.acp_impl.agent.load_agent_specs") as mock_load_specs, \
        ...
        # Call the method
        conversation = acp_agent._setup_acp_conversation(session_id)
...
async def test_send_streaming_chunk_error_handling(acp_agent, mock_connection):
    ...
    try:
        ...
        pytest.fail(f"_send_streaming_chunk should handle errors gracefully, but raised: {e}")