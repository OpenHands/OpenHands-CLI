...
def test_handle_multiline_mode_updates_indicator_and_refreshes(dummy_app, monkeypatch):
    ...
    assert widget.mode_indicator == "\\[Multi-line: Ctrl+J to submit • Ctrl+X for custom editor]"
    update_text_mock.assert_called_once()
    ...
    assert widget.mode_indicator == "\\[Ctrl+L for multi-line • Ctrl+X for custom editor]"
    update_text_mock.assert_called_once()
