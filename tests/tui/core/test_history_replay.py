"""Tests for history replay with pagination."""
...
class TestHistoryReplayState:
    """Tests for HistoryReplayState dataclass."""

    def test_empty_events(self):
        """Empty events list should have no more to load."""
        state = HistoryReplayState(events=[], displayed_count=0)
    ...
    def test_has_more_with_events(self):
        ...
        state = HistoryReplayState(events=events, displayed_count=0)
        assert state.has_more
        assert state.remaining_count == 10

    def test_has_more_after_partial_display(self):
        """Should report has_more after partial display."""
        events = [Mock() for _ in range(10)]
        state = HistoryReplayState(events=events, displayed_count=5)
        assert state.has_more
        assert state.remaining_count == 5

    def test_no_more_when_all_displayed(self):
        """Should not have more when all events displayed."""
        events = [Mock() for _ in range(10)]
        state = HistoryReplayState(events=events, displayed_count=10)
        assert not state.has_more
        assert state.remaining_count == 0
    ...
    def test_get_initial_page_returns_last_n_events(self):
        ...
        state = HistoryReplayState(events=events, page_size=5)
    ...
    def test_default_page_size(self):
        ...
        state = HistoryReplayState(events=events)
...
class TestReplayEventsToVisualizer:
    """Tests for replay_events_to_visualizer function."""

    def test_replays_all_events(self):
        """Should call on_event for each event."""
        events = [Mock() for _ in range(5)]
        visualizer = MagicMock()

        replay_events_to_visualizer(events, visualizer)
    ...
    def test_empty_events_no_calls(self):
        ...
        replay_events_to_visualizer([], visualizer)
...
class TestPaginationIntegration:
    ...
    def test_full_pagination_workflow(self):
        ...
        state = create_history_state(events, page_size=5)
