...
class TestLogoutCommand:
    ...
    def test_logout_command_specific_server_logged_in(self):
        ...
        with patch("openhands_cli.auth.logout_command.TokenStorage") as mock_storage_class:
            with patch("openhands_cli.auth.logout_command._p") as mock_print:
                ...
                assert any("Successfully logged out from" in call and server_url in call for call in print_calls)
    ...
    def test_logout_command_specific_server_not_logged_in(self):
        ...
        with patch("openhands_cli.auth.logout_command.TokenStorage") as mock_storage_class:
            with patch("openhands_cli.auth.logout_command._p") as mock_print:
                ...
                # Check that appropriate messages were printed
                print_calls = [call[0][0] for call in mock_print.call_args_list]
                assert any("not logged in to" in call and server_url in call for call in print_calls)
    ...
    def test_logout_command_global_not_logged_in(self):
        ...
        with patch("openhands_cli.auth.logout_command.TokenStorage") as mock_storage_class:
            with patch("openhands_cli.auth.logout_command._p") as mock_print:
                ...
                # Check that appropriate messages were printed
                print_calls = [call[0][0] for call in mock_print.call_args_list]
                assert any("not logged in to any servers" in call for call in print_calls)
    ...
    def test_logout_command_exception_handling(self):
        ...
        with patch("openhands_cli.auth.logout_command.TokenStorage") as mock_storage_class:
            with patch("openhands_cli.auth.logout_command._p") as mock_print:
                ...
                print_calls = [call[0][0] for call in mock_print.call_args_list]
                assert any("Unexpected error during logout" in call and "Unexpected error" in call for call in print_calls)
    ...
    def test_logout_command_storage_exception(self):
        ...
        with patch("openhands_cli.auth.logout_command.TokenStorage") as mock_storage_class:
            with patch("openhands_cli.auth.logout_command._p") as mock_print:
                ...
                # Check that error message was printed
                print_calls = [call[0][0] for call in mock_print.call_args_list]
                assert any("Unexpected error during logout" in call and "Storage error" in call for call in print_calls)
