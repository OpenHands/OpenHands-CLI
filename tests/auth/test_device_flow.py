...
class TestDeviceFlowClient:
    ...
    async def test_start_device_flow_http_error(self):
        ...
        with patch.object(client, "post") as mock_post:
            ...
            with pytest.raises(DeviceFlowError, match="Failed to start device flow: Network error"):
    ...
    async def test_poll_for_token_access_denied(self):
        ...
        with patch.object(client, "post") as mock_post:
            ...
            with pytest.raises(DeviceFlowError, match="User denied the authorization request"):
                await client.poll_for_token("device123", 1)
    ...
    async def test_poll_for_token_unknown_error(self):
        ...
        with patch.object(client, "post") as mock_post:
            ...
            with pytest.raises(DeviceFlowError, match="Authorization error: unknown_error - Something went wrong"):
                await client.poll_for_token("device123", 1)
    ...
    async def test_poll_for_token_network_error(self):
        ...
        with patch.object(client, "post") as mock_post:
            ...
            with pytest.raises(DeviceFlowError, match="Network error during token polling"):
                await client.poll_for_token("device123", 1)
    ...
    async def test_poll_for_token_invalid_json_response(self):
        ...
        with patch.object(client, "post") as mock_post:
            ...
            with pytest.raises(DeviceFlowError, match="Unexpected response from server: 500"):
                await client.poll_for_token("device123", 1)
    ...
    async def test_poll_for_token_timeout(self):
        ...
        with patch.object(client, "post") as mock_post:
            ...
            with patch("asyncio.sleep"):
                with pytest.raises(DeviceFlowError, match="Timeout waiting for user authorization"):
                    await client.poll_for_token("device123", 1)
    ...
    async def test_authenticate_success(self):
        ...
        with patch.object(client, "start_device_flow") as mock_start:
            mock_start.return_value = ("device123", "USER123", "https://example.com/device", 5)
            
            with patch.object(client, "poll_for_token") as mock_poll:
                mock_poll.return_value = {"access_token": "token123", "token_type": "Bearer"}
