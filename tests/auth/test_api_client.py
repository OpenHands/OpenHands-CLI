...
class TestOpenHandsApiClient:
    ...
    async def test_get_json_http_error(self):
        ...
        with patch.object(client, "get") as mock_get:
            ...
            with pytest.raises(ApiClientError, match="Request to '/test' failed: Network error"):
...
class TestHelperFunctions:
    ...
    def test_create_agent_from_settings(self):
        ...
        with patch("openhands_cli.auth.api_client.LLM") as mock_llm_class:
            with patch("openhands_cli.auth.api_client.LLMSummarizingCondenser") as mock_condenser_class:
                with patch("openhands_cli.auth.api_client.Agent") as mock_agent_class:
                    with patch("openhands_cli.auth.api_client.get_default_tools") as mock_get_tools:
                        mock_llm = MagicMock()
    ...
    async def test_fetch_user_data_after_oauth_success(self):
        ...
        with patch("openhands_cli.auth.api_client.OpenHandsApiClient") as mock_client_class:
            with patch("openhands_cli.auth.api_client.create_agent_from_settings") as mock_create_agent:
                with patch("openhands_cli.auth.api_client.save_agent_configuration") as mock_save_agent:
                    with patch("openhands_cli.auth.api_client._p") as mock_print:
                        ...
                        expected_result = {
                            ...
                            "settings": {"llm_model": "gpt-4o", "agent": "CodeActAgent"}
                        }
