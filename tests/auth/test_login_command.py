...
class TestLoginCommand:
    ...
    async def test_fetch_user_data_with_context_already_logged_in_success(self):
        ...
        with patch("openhands_cli.auth.login_command.fetch_user_data_after_oauth") as mock_fetch:
            with patch("openhands_cli.auth.login_command._p") as mock_print:
                mock_fetch.return_value = {"llm_api_key": "llm-key", "settings": {}}
                
                await _fetch_user_data_with_context(server_url, api_key, already_logged_in=True)
    ...
    async def test_fetch_user_data_with_context_new_login_success(self):
        ...
        with patch("openhands_cli.auth.login_command.fetch_user_data_after_oauth") as mock_fetch:
            with patch("openhands_cli.auth.login_command._p") as mock_print:
                ...
                await _fetch_user_data_with_context(server_url, api_key, already_logged_in=False)
    ...
    async def test_fetch_user_data_with_context_api_error(self):
        ...
        with patch("openhands_cli.auth.login_command.fetch_user_data_after_oauth") as mock_fetch:
            with patch("openhands_cli.auth.login_command._p") as mock_print:
                ...
                print_calls = [call[0][0] for call in mock_print.call_args_list]
                assert any("Warning" in call and "API error" in call for call in print_calls)
    ...
    async def test_login_command_existing_token(self):
        ...
        with patch("openhands_cli.auth.login_command.TokenStorage") as mock_storage_class:
            with patch("openhands_cli.auth.login_command._fetch_user_data_with_context") as mock_fetch:
                with patch("openhands_cli.auth.login_command._p") as mock_print:
    ...
    async def test_login_command_new_login_success(self):
        ...
        with patch("openhands_cli.auth.login_command.TokenStorage") as mock_storage_class:
            with patch("openhands_cli.auth.login_command.authenticate_with_device_flow") as mock_auth:
                with patch("openhands_cli.auth.login_command._fetch_user_data_with_context") as mock_fetch:
                    with patch("openhands_cli.auth.login_command._p") as mock_print:
                        mock_storage = MagicMock()
                        mock_storage_class.return_value = mock_storage
                        mock_storage.get_api_key.return_value = None  # No existing token
            ...
            with patch("openhands_cli.auth.login_command.authenticate_with_device_flow") as mock_auth:
                with patch("openhands_cli.auth.login_command._fetch_user_data_with_context") as mock_fetch:
                    with patch("openhands_cli.auth.login_command._p") as mock_print:
                        ...
                        mock_auth.return_value = {"access_token": "", "token_type": "Bearer"}
            ...
            with patch("openhands_cli.auth.login_command.authenticate_with_device_flow") as mock_auth:
                with patch("openhands_cli.auth.login_command._fetch_user_data_with_context") as mock_fetch:
                    with patch("openhands_cli.auth.login_command._p") as mock_print:
                        ...
                        print_calls = [call[0][0] for call in mock_print.call_args_list]
                        assert any("Successfully logged in" in call for call in print_calls)
