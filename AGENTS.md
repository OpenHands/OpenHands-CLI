...
- Until `openhands-sdk`, `openhands-tools`, and `openhands-workspace` 1.12.x ship this settings work in a normal release, pin CLI cross-repo settings work to the SDK issue branch via `[tool.uv.sources]` so CI exercises the shared schema code instead of the older released packages.
